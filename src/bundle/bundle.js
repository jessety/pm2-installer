...
async function compare(bundle) {
  ...
  if (bundle.package.version !== version) {
    console.warn(`${colors.yellow('WARNING')}: Bundle was created with ${bundle.package.name} ${colors.cyan(bundle.package.version)}, but this is ${colors.green(version)}.`);
  }

  if (bundle.node !== node) {
    console.warn(`${colors.yellow('WARNING')}: Bundle was created with Node ${colors.cyan(bundle.node)} but this is ${colors.green(node)}.`);
  }

  if (bundle.npm !== npm) {
    console.warn(`${colors.yellow('WARNING')}: Bundle was created with npm ${colors.cyan(bundle.npm)} but this is ${colors.green(npm)}.`);
  }

  if (bundle.os.platform !== platform || bundle.os.release !== release) {
    console.warn(`${colors.yellow('WARNING')}: Bundle was created on ${colors.cyan(bundle.os.platform)}-${colors.cyan(bundle.os.release)} but this is ${colors.green(platform)}-${colors.green(release)}.`);
  }
